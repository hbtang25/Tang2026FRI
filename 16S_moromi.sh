...
# 0. Set environment variables
    ...
    wd=~/project/amplicon/16S_moromi
    sw=/opt/bin
    cd ${wd}
...
## 1.4 Dereplication
   vsearch --derep_fulllength temp/filtered.fa \
      --output temp/uniques.fa --relabel Uni --minuniquesize 43 --sizeout
...
## 1.7 Species annotation – excluding plastids and non-bacterial organisms
    vsearch --sintax result/04_raw_output/otus.fa --db ${db}/usearch/rdp_16s_v16.fa \
      --tabbedout result/04_raw_output/otus.sintax --sintax_cutoff 0.4
   
    # R script for selecting bacteria and archaea (eukaryotes)
    Rscript ~/scripts/otutab_filter_nonBac.R \
      --input result/04_raw_output/otutab.txt \
      --taxonomy result/04_raw_output/otus.sintax \
      --output result/otutab.txt\
      --stat result/04_raw_output/otutab_nonBac.stat \
      --discard result/04_raw_output/otus.sintax.discard
...
# 4. Functional Prediction
    ...
    picrust2_pipeline.py -s otus.fa -i otutab.txt -o 10_picrust2 -p 32
      
    # Add descriptsions as new column in gene family and pathway abundance tables (details)
    add_descriptsions.py -i 10_picrust2/EC_metagenome_out/pred_metagenome_unstrat.tsv.gz -m EC \
      -o 10_picrust2/EC_metagenome_out/pred_metagenome_unstrat_descrip.tsv.gz
      
    add_descriptsions.py -i 10_picrust2/KO_metagenome_out/pred_metagenome_unstrat.tsv.gz -m KO \
      -o 10_picrust2/KO_metagenome_out/pred_metagenome_unstrat_descrip.tsv.gz
  
    add_descriptsions.py -i 10_picrust2/pathways_out/path_abun_unstrat.tsv.gz -m METACYC \
      -o 10_picrust2/pathways_out/path_abun_unstrat_descrip.tsv.gz
    
    gunzip 10_picrust2/EC_metagenome_out/pred_metagenome_unstrat_descrip.tsv.gz
    gunzip 10_picrust2/KO_metagenome_out/pred_metagenome_unstrat_descrip.tsv.gz
    gunzip 10_picrust2/pathways_out/path_abun_unstrat_descrip.tsv.gz
